...
- [ ] **SUPR-01**: After an alert fires for a given entity+alert-type, subsequent notifications are suppressed for a configurable repeat interval (default 4 hours) rather than firing every polling cycle
- [ ] **SUPR-02**: The alert repeat interval is user-configurable from the HA config UI
- [ ] **SUPR-03**: Suppression state resets when the alert condition clears — if the condition resolves and re-triggers, a fresh notification fires immediately
