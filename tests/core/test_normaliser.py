...
from custom_components.behaviour_monitor.core.normaliser import (
    Normaliser,
    NormaliserConfig,
)
...
def test_motion_rising_edge_is_presence_and_retrigger_is_merged():
    ...
    assert (
        n.handle("binary_sensor.k", Category.MOTION, "Kitchen", "on", "off", _t(62))
        == []
    )
    # retrigger inside the 90 s window extends the burst
    assert (
        n.handle("binary_sensor.k", Category.MOTION, "Kitchen", "off", "on", _t(70))
        == []
    )
    assert (
        n.handle("binary_sensor.k", Category.MOTION, "Kitchen", "on", "off", _t(132))
        == []
    )
...
def test_contact_open_close_with_duration():
    ...
    assert (
        ev[0].kind == EventKind.CLOSE
        and ev[0].duration_s == 45.0
        and not ev[0].is_activity
    )
...
def test_light_and_other():
    n = _n()
    assert (
        n.handle("light.l", Category.LIGHT, "Lounge", "off", "on", _t(0))[0].kind
        == EventKind.LIGHT_ON
    )
    assert (
        n.handle("light.l", Category.LIGHT, "Lounge", "on", "off", _t(1))[0].kind
        == EventKind.LIGHT_OFF
    )
...
def test_same_state_replay_emits_nothing():
    n = _n()
    assert (
        n.handle("binary_sensor.k", Category.MOTION, "Kitchen", "on", "on", _t(0)) == []
    )
...
def test_unavailable_transitions_are_health_not_activity():
    n = _n()
    ev = n.handle(
        "binary_sensor.k", Category.MOTION, "Kitchen", "off", "unavailable", _t(0)
    )
    assert ev == [
        HealthEvent(
            "binary_sensor.k", Category.MOTION, "Kitchen", _t(0), available=False
        )
    ]
    # coming back with state on is a restore, not the person
    ev = n.handle(
        "binary_sensor.k", Category.MOTION, "Kitchen", "unavailable", "on", _t(10)
    )
    assert ev == [
        HealthEvent(
    ...
    assert (
        n.handle(
            "binary_sensor.b", Category.MOTION, "Bath", "unavailable", "unknown", _t(31)
        )
        == []
    )
...
def test_numeric_plug_before_min_samples_uses_minimum_seen():
    n = Normaliser(NormaliserConfig(plug_min_samples=10, plug_margin_w=5.0))
    assert n.handle(
        "sensor.kettle", Category.PLUG, "Kitchen", None, "0", _t(0)
    )  # health only
...
def test_switch_plug_uses_on_off():
    n = _n()
    assert (
        n.handle("switch.p", Category.PLUG, "Loft", "off", "on", _t(0))[0].kind
